      ...
      <SafeAreaView edges={["bottom", "left", "right"]} className="flex-1">
        ...
        >
          <View className="flex-col gap-4">
            {loading && (
              <Text className="text-white text-center">Ładowanie szczytów...</Text>
            )}
            ...
            {!loading && !error && peaks && peaks.length > 0 ? (
              peaks.map((peak: any, index: number) => (
                <View key={index} className="w-full bg-white/10 rounded-2xl p-5 border-l-4 border-yellow-500">
                  <View className="flex-row items-center justify-between mb-3">
                    <View className="flex-row items-center gap-3">
                      <View className="bg-yellow-500/20 w-12 h-12 rounded-full items-center justify-center">
                        ...
                        <Text className=" absolute top-5 right-4.1 text-white font-semibold text-xs">{peak.times_visited} </Text>
                      </View>
                      <View>
                        <Text className="text-2xl font-bold text-white">{peak.peak_name || "Nieznany szczyt"}</Text>
                        <Text className="text-sm text-white/70">{peak.elevation || "?"} m n.p.m.</Text>
                      </View>
                    </View>
                  ...
                  <View className="flex-row items-center gap-2 mt-2">
                    ...
                    <Text className="text-white/70 text-sm">
                      {peak.last_visited ? new Date(peak.last_visited).toLocaleDateString('pl-PL', {
                        weekday: 'long', 
                        year: 'numeric', 
                        month: 'long', 
                        day: 'numeric' 
