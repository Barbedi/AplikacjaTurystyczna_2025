...
const MyPeaksUser = () => {
  ...
  const fetchMyPeaks = async () => {
    ...
    try {
      setLoading(true);
      const response = await userpeaksService.getUserPeaks(currentUser.id, pageData.page); 
  ...
  <div className="flex flex-col  mt-4">
    {loading ? (
      ...
      <div className="flex flex-col items-center justify-center w-full p-4 bg-white/10 backdrop-blur-lg rounded-lg shadow-md">
        <h2 className="text-lg font-lora text-white">Nie masz jeszcze żadnych szczytów!</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 flex-grow">
          {myPeaks.map((peak) => (
            ...
            >
              ...
              <p className="text-sm text-gray-300">Data: {formatDate(peak.last_visited)}</p>
              <a
