...
const ElevationProfile: React.FC<ElevationData> = ({ route, onHoverPoint }) => {
  ...
  const { coords, elevationData, totalDistanceKm, yDomain } = useMemo(() => {
    if (!route || !route.features || route.features.length === 0) {
      return { coords: [], elevationData: [], totalDistanceKm: 0, yDomain: [0, 100] };
    }
    ...
    const maxPoints = 500; 
    const step = coords.length > maxPoints ? Math.ceil(coords.length / maxPoints) : 1;
    ...
    const elevationData = sampledCoords.map(
      (coord: [number, number, number], idx: number) => {
        ...
        return {
          distance: parseFloat(
            ((originalIndex / (coords.length - 1)) * totalDistanceKm).toFixed(2),
          ),
          elevation: coord[2],
          originalIndex, 
        };
    ...
    const elevations = elevationData.map(d => d.elevation).filter(e => !isNaN(e));
    const minElevation = Math.min(...elevations);
    const maxElevation = Math.max(...elevations);
    const margin = (maxElevation - minElevation) * 0.1; 
    const yDomain = [
      Math.max(0, minElevation - margin),
      maxElevation + margin
    ];
  ...
  const handleMouseMove = useCallback(
    (state: { activeTooltipIndex?: number | string | null }) => {
      if (state && state.activeTooltipIndex != null && onHoverPoint && elevationData.length > 0) {
        const index = state.activeTooltipIndex as number;

        if (lastIndex.current !== index && index >= 0 && index < elevationData.length) {
    ...
    <div className="w-full h-64">
      ...
      <div className="text-xs text-white/70 mt-1 text-center">
        {elevationData.length < coords.length && (
          <span>Pokazano {elevationData.length} z {coords.length} punktów</span>
        )}
      </div>
    </div>
  );
